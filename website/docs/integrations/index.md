...
- **[Hermes MCP in Cursor IDE](/guides/cursor-mcp)** — Cursor already has a coding agent. Point it at `hermes mcp serve` (messaging-bridge tools) plus the QENEX, project-scoped Supabase, Pipedrive, and Sourcegraph HTTP MCP servers. Do not run Hermes as an ACP agent inside Cursor.
